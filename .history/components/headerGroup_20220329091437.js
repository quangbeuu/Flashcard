...
class Header {
  constructor() {
    ...
    this.$userModalContainer.setAttribute(
      "class",
      "absolute bg-white right-0 shadow-card rounded-[8px] max-w-[200px]  overflow-hidden"
    );
    this.$userModalInfoWrap = document.createElement("div");
    this.$userModalInfoWrap.setAttribute(
      "class",
      "flex items-center w-full border-b-[1px] border-black px-[20px] py-[15px]"
    );
    ...
    this.$userImage.setAttribute(
      "class",
      "w-8 h-8 rounded-full object-cover cursor-pointer inline-block"
    );
    this.$userInfo = document.createElement("div");
    this.$userInfo.setAttribute("class", "ml-[10px]");
  ...
  render(container) {
    ...
    this.$userModalInfoWrap.appendChild(this.$userImageDiv);
