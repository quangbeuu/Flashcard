    ...
    onSnapshot(q, (snapshot) => {
      snapshot.docChanges().forEach((change) => {
        ...
        if (change.type === "added") {
          cardArr.push(cardDoc);
        }
        cardArr.sort((a, b) => a.createdAt - b.createdAt);
        cardArr.forEach((card) => {
          const carditem = new CardItem(card, cardId);
          carditem.$cardWrap.id = cardId;
          this.$cardList.appendChild(carditem.render());
        });
      });
  };
