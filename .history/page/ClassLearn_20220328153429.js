...
class ClassLearn {
  constructor(data) {
    ...
    this.$createSetButtonWrap = document.createElement("div");
    this.$createSetButtonWrap.setAttribute(
      "class",
      "flex items-center justify-center"
    );
    this.$createSetButton = document.createElement("a");
    this.$createSetButton.innerText = "+ Create a new set";
    this.$createSetButton.setAttribute(
      "class",
      "text-[16px] font-bold text-center tracking-[1px] cursor-pointer hover:text-[#ffcd1f] transition ease-in duration-100ms border-b-[3px] border-transparent hover:border-[#ffcd1f] uppercase"
    );

    this.$createSetButton.addEventListener("click", this.gotoCreateSetPage);
    this.getDocById();
  }
  // Add a set
  addASet = () => {
  ...
  gotoCreateSetPage = () => {
    const createSetPage = new CreateSet(this.roomId);
  ...
  render(container) {
    ...
    // Create Set
    this.$createSetModal.$modalInfroWrap.appendChild(this.$createSetButtonWrap);
    this.$createSetButtonWrap.appendChild(this.$createSetButton);
